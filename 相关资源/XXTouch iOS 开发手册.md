...
- ### 屏幕找图 (**screen\.find\_image**)
    ...
    - 示例  
        [XXT 取色器 1.0.33 Windows 版.7z](https://raw.githubusercontent.com/havonz/XXTouchDebs/master/%E7%9B%B8%E5%85%B3%E8%B5%84%E6%BA%90/XXT%20%E5%8F%96%E8%89%B2%E5%99%A8%201.0.33%20For%20Windows.7z)
        ```lua
...
- ### \! 将对话框弹出来并返回用户的选择 (**:show**)
    ...
    - 说明  
        > 将对话框弹出来并返回用户的选择  
        > 当对话框设置了配置保存（:set_config\(配置名\)）的情况下，按下 **提交** 会保存配置，按下右上的 **×** 或超时则不会保存  
...
- ### 清理系统缓存 (**clear\.caches**)
    ...
    - 示例  
        ...
        clear.caches{no_uicache = true} -- 支持不使用 uicache 清理，uicache 耗时很长，可使用 os.execute('su mobile -c uicache') 来代替
        ```
...
- ### 比较两个版本号大小 (**string\.compare\_version**)
    ...
    - 示例
        ```lua
        -- "1.5" 和 "1.05" 和 "1.005" 是同一个版本
        -- "1.5" 和 "1.50" 和 "1.500" 是三个不同的版本号
        -- "1.11" 大于 "1.5"
        -- "1.52" 等于 "1.52.0"
...
- ### 发起 GET 请求 (**http\.get**)
    ...
    - 示例  
        ...
        -- no_escape（URL不ESCAPE） 参数示例，下面的例子与上面例子等效
...
- ### 获取一个文本文件指定行的数据 (**file\.get\_line**)
    ...
    - 说明  
        > 获取一个文本文件指定行的数据  
        > **这个函数将自动剔除 UTF8\-BOM**（[百度搜索 UTF8-BOM 查看更多资料](https://www.baidu.com/s?wd=UTF8-BOM)）  
...
- #### ~~伪装GPS位置 (**gps\.fake**)~~
    ...
    - 说明  
        > **需要导入 [地理位置伪装插件（点击下载）](http://xxtouch.oss-cn-shanghai.aliyuncs.com/XXT%E5%9C%B0%E7%90%86%E4%BD%8D%E7%BD%AE%E4%BC%AA%E8%A3%85%E6%8F%92%E4%BB%B6.zip) 方可使用**  
...
- Windows 平台工具
    > - [XXT_编辑器-Windows_版.zip](https://raw.githubusercontent.com/havonz/XXTouchDebs/master/%E7%9B%B8%E5%85%B3%E8%B5%84%E6%BA%90/XXTStudio-Win-2025111323.zip)  
    > - [XXT_编辑器-macOS_版.zip](https://raw.githubusercontent.com/havonz/XXTouchDebs/master/%E7%9B%B8%E5%85%B3%E8%B5%84%E6%BA%90/XXTStudio-macOS-2025111323.zip)  
    > - [XXT 取色器 1.0.33 Windows 版.7z](https://raw.githubusercontent.com/havonz/XXTouchDebs/master/%E7%9B%B8%E5%85%B3%E8%B5%84%E6%BA%90/XXTColorPicker%201.0.33%20For%20Windows.7z)  
    > - [XXT 取色器 1.0.33 macOS 版 (depends JRE).zip](https://raw.githubusercontent.com/havonz/XXTouchDebs/master/%E7%9B%B8%E5%85%B3%E8%B5%84%E6%BA%90/XXTColorPicker%201.0.33%20For%20macOS%20(depends%20JRE).zip)  
...
- 只有 `nil` 和 `false` 是逻辑假，其它值都是逻辑真，包括 `0 也是逻辑真`
